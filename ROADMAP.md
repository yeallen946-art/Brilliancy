...
## M-CN — 微信小程序(大陆并行轨,PRD §12.6 节奏)

骨架已搭(`miniprogram/`,2026-06-11):今日页 + 猜棋循环 + 总结/分享,零引擎架构(TECH_SPEC §11),内容复用管线双语 JSON。剩余工作按解锁顺序:

1. **现在~M5 期间(不阻塞海外线)**:Jerry 公司侧并行推进 ICP/教育备案、境内内容域名;骨架在开发者工具里跑通真机预览。
2. **M5 海外 TestFlight 验证内核之后**:补 streak/本地成绩(wx.storage)、cburnett 棋子图、服务通知召回;`BASE_URL` 切境内域名;教育类目提审(灰色点:虚拟支付是否需版号,办主体方确认)。
3. **跑出量后**:虚拟支付(归档/题库收费,苹果抽 15%)、分包、按 PRD §12.2 评估大陆原生 App Store。

**验收(骨架阶段):** 开发者工具中完整玩完一局每日挑战——自动走子、点选猜棋、三档揭示卡(中文讲解)、分享卡不剧透。
